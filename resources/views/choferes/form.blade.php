...
<section class="content">
	<div class="col-md-12">
		...
		@if($choferes->cedcho == NULL)
			<div class="panel panel-default">
				<div class="panel-heading">
					<h3 class="panel-title">Nuevo Chofer..</h3>
				</div>
				<div class="panel-body">
					<div class="table-container">
						<form method="POST" action="{{ route('choferes.store') }}" role="form">
							{{ csrf_field() }}
							...
							<div class="form-group" align="center">
								<a class="button" href="{{ route('choferes.index') }}">Atrás</a>
		...
		@else
			<div class="panel panel-default">
				<div class="panel-heading">
					<h3 class="panel-title">Edición de Chofer..</h3>
				</div>
				<div class="panel-body">
					<div class="table-container">
						<form method="POST" action="{{ route('choferes.update',$choferes->cedcho) }}"  role="form">
							{{ csrf_field() }}
							<input name="_method" type="hidden" value="PATCH">

							<div class="form-group">
								<label for="nombre">Nombres</label>
								<input type="text" name="nombre" id="nombre" class="form-control" value="{{$choferes->nombre}}" placeholder="Nombre y Apellido">
							</div>
							...
							<div class="form-group">
								...
								<select class="form-control" name="idpais" id="idpais">
									<option value=""disabled="true" selected="true">Seleccione un País</option>
									  @foreach($pais as $pais)
									  	@if( $choferes->idpais  ==  $pais->idpais )
											<option value="{{ $choferes->idpais }}" selected>{{ $pais->pais }}</option>
									  	@else
											<option value="{{ $pais->idpais }}">{{ $pais->pais }}</option>
										@endif
									  @endforeach
							...
							<div class="form-group">
								<label for="tel">Teléfonos</label>
								<input type="tel" name="tel" id="tel" class="form-control" value="{{ $choferes->tel }}" placeholder="Indique el Teléfono. Emeplo: +58 424-6883287">
							</div>	

							<div class="form-group">
								<label for="email">Email</label>
								<input type="email" class="form-control" name="email" id="email" aria-describedby="emailHelp" value="{{ $choferes->email }}" placeholder="Dirección de correo electrónico">	
							</div>

							<div class="form-group">
								<label for="obs">Observaciones</label>
								<textarea name="obs" class="form-control" placeholder="Observaciones generales">{{ $choferes->obs }}</textarea>
							</div>

							<div class="form-group" align="center">
								<a class="button info" href="{{ route('choferes.index') }}">Atrás</a>
